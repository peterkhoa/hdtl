...
namespace Library
{
    ...
    public static class Time
    {
        ...
        public static string GetDuration(DateTime dt)
        {
            TimeSpan d = DateTime.Now - dt;

            if (d.TotalSeconds < 60)
                return Math.Ceiling(d.TotalSeconds).ToString() + "  giây trước";
            if (d.TotalMinutes < 60)
                return Math.Round(d.TotalMinutes).ToString() + " phút trước";
            if (d.TotalHours < 24)
                return Math.Round(d.TotalHours).ToString() + " giờ trước";

            // if (d.TotalDays < 30)
            return Math.Round(d.TotalDays).ToString() + " ngày trước";



            // return   Math.Round(d.TotalDays / 30).ToString() + " tháng trước";
        }

        public static string GetRemainDuration(DateTime dt)
        {
            TimeSpan d = dt -DateTime.Now ;

            if (d.TotalSeconds < 60)
                return Math.Ceiling(d.TotalSeconds).ToString() +  " giây";
            if (d.TotalMinutes < 60)
                return Math.Round(d.TotalMinutes).ToString() + " phút";
            if (d.TotalHours < 24)
                return Math.Round(d.TotalHours).ToString() + " giờ";

            // if (d.TotalDays < 30)
            return Math.Round(d.TotalDays).ToString() + " ngày";
        ...
        {
            while (time.DayOfWeek.ToString() !="Monday")
            {
               time=  time.AddDays(-1);
            }
        ...
        public static DateTime getSaturdayLast(DateTime time)
        {
            while (time.DayOfWeek.ToString() != "Saturday")
            {
                time = time.AddDays(-1);
            }
            return time.Date;
        }
        public static DateTime getNextMonday(DateTime time)
        {


            return getMonday(time).AddDays(7);
        }

        public static DateTime getMonthFirst(DateTime time)
        {
            while (time.Day != 1)
            {
                time = time.AddDays(-1);
